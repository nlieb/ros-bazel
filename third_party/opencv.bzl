...
load("//bzl:generate_file.bzl", "generate_file")
load("//bzl:template_file.bzl", "template_file")
...
""".format(opt = opt[0].upper())  # FIXME(nlieb): should not need to add CV_CPU_DISPATCH_FEATURES
...
#define _CUSTOM_HAL_INCLUDED_
    ...
    generate_file(
        ...
        content = select({
            "@ros_bazel//bzl/constraints:arm": _format_config("armeabihf", config.get("opts", [])),
            "@ros_bazel//bzl/constraints:x86_64": _format_config("x86_64", config.get("opts", [])),
        }),
    )
